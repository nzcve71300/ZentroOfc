...
async function removeSnb2xServer() {
  try {
    console.log('🗑️ Removing SNB 2X server...');
    
    // Step 1: Find the SNB 2X server by IP and port
    console.log('\n📋 Step 1: Finding SNB 2X server...');
    ...
    if (servers.length === 0) {
      console.log('❌ No server found with IP 81.0.247.39:29816');
      ...
      if (nicknameServers.length === 0) {
        console.log('❌ No SNB 2X server found by nickname either');
        return;
      } else {
    ...
    console.log('\n🗑️ Removing SNB 2X server from rust_servers...');
    ...
    if (deleteResult.affectedRows > 0) {
      console.log(`✅ Successfully removed SNB 2X server: ${server.nickname}`);
      console.log('\n🎯 SNB 2X SERVER REMOVAL COMPLETE!');
      ...
      console.log('❌ Failed to remove SNB 2X server');
    }
    
  } catch (error) {
    console.error('❌ Error removing SNB 2X server:', error);
  } finally {
    await pool.end();
  }
}

...
async function restoreMissingServerLinks() {
  let connection;
  ...
  try {
    ...
    console.log('🔍 Finding players with missing server links...');

    // Get all servers
    const [servers] = await connection.execute(`
      SELECT id, nickname, guild_id FROM rust_servers ORDER BY guild_id, nickname
    `);
    ...
    // For each server, find players who are linked on other servers but missing on this one
    for (const targetServer of servers) {
      console.log(`\n🔍 Checking server: ${targetServer.nickname} (${targetServer.id})`);
      
      // Find players who are linked on other servers in the same guild but not on this server
      const [missingPlayers] = await connection.execute(`
        ...
        WHERE p.guild_id = ? 
          ...
          AND p.discord_id NOT IN (
            SELECT discord_id 
            FROM players 
            WHERE guild_id = ? AND server_id = ? AND is_active = TRUE AND discord_id IS NOT NULL
          )
        GROUP BY p.discord_id, p.ign, p.normalized_ign, p.guild_id
        ORDER BY p.ign
      `, [targetServer.guild_id, targetServer.id, targetServer.guild_id, targetServer.id]);
    ...
    console.log(`📊 Total players restored across all servers: ${totalRestored}`);
...
module.exports = restoreMissingServerLinks;

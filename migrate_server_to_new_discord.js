...
async function migrateServerToNewDiscord() {
  const oldDiscordId = '1403300500719538227';
  const newDiscordId = '1414692371144376395';
  
  try {
    console.log('🔄 Starting server migration to new Discord...');
    console.log(`📤 Source Discord: ${oldDiscordId}`);
...
if (shouldConfirm) {
  console.log('🚀 CONFIRMED: Proceeding with migration...\n');
  executeMigration();
} else {
  migrateServerToNewDiscord();
}
...
async function executeMigration() {
  ...
  try {
    console.log('🔄 EXECUTING SERVER MIGRATION...\n');

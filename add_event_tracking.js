...
// Add the event tracking functionality
...
// Find where to insert the event tracking code (after the existing functions)
const functionEndPattern = /}\s*$/;
const updatedContent = rconContent.replace(functionEndPattern, `${eventTrackingCode}\n}`);

// Now add the event tracking trigger in the message handling section
const eventTriggerCode = `
        // Event tracking trigger
        if (message.includes('d11_quick_chat_orders_slot_7')) {
          console.log('[EVENT TRACKING] Triggered by player:', username);
          await handleEventTracking(client, guildId, serverName, ip, port, password, username);
          return;
        }
`;

// Find the message handling section and add the event trigger
const messageHandlingPattern = /\/\/ Handle kit emotes/;
if (messageHandlingPattern.test(updatedContent)) {
  ...
  const backupPath = rconPath + '.backup12';

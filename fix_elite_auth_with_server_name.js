...
console.log('🔧 Fixing elite kit authorization using serverName...\n');
...
// Create the correct elite authorization logic using serverName
const eliteAuthLogic = `
        ...
        if (kitKey.startsWith('ELITEkit')) {
          ...
          const [serverResult] = await pool.query(
            'SELECT id FROM rust_servers WHERE server_name = ?',
            [serverName]
          );
...
if (duplicateCheckPattern.test(cleanedContent)) {
  ...
  const backupPath = rconPath + '.backup4';

...
async function testCategoryAutocomplete() {
  try {
    ...
    // Test 3: Test the exact query that addShopItem uses
    console.log('\n🔧 Testing category autocomplete query...');
    const guildId = servers[0].guild_id;
    const serverNickname = servers[0].nickname;
    ...
    if (result.length > 0) {
      ...
      console.log('❌ Category autocomplete query returned no results');
    }

...
// Find and fix the event tracking trigger to get the correct player name
...
// Find and fix the handleEventTracking function to properly parse the response
...
// Find and fix the parseEventTimes function to properly handle the response format
...
if (eventTriggerPattern.test(updatedContent)) {
  ...
  console.log('✅ Fixed event trigger with proper player name extraction');
} else {
...
if (parseEventTimesPattern.test(updatedContent)) {
  ...
  console.log('✅ Fixed parseEventTimes function with better debugging');
} else {
  console.log('❌ Could not find parseEventTimes pattern');
}

// Backup the original file
const backupPath = rconPath + '.backup17';
fs.copyFileSync(rconPath, backupPath);
console.log('✅ Created backup:', backupPath);

// Write the updated content
fs.writeFileSync(rconPath, updatedContent);
console.log('✅ Fixed final event tracking issues in src/rcon/index.js');

console.log('\n✅ Final event tracking issues fixed!');

...
async function fixMissingPlayerRecordsV2() {
  console.log('🔧 FIXING MISSING PLAYER RECORDS - V2');
  console.log('=======================================\n');
  ...
  try {
    ...
    // Step 2: Find all Discord users who have economy records
    ...
    for (const record of economyRecords) {
      if (record.discord_id) {
        // Check if player exists on Dead-ops (active or inactive)
        const [deadOpsPlayer] = await connection.execute(
          'SELECT * FROM players WHERE server_id = ? AND discord_id = ?',
          [deadOpsServer.id, record.discord_id]
        );
        
        // Check if player exists on USA-DeadOps (active or inactive)
    ...
    // Step 3: Create missing player records or activate existing ones
    ...
    for (const player of missingPlayers) {
      try {
        if (player.missingDeadOps) {
          // Check if there's an inactive record first
          const [inactiveRecord] = await connection.execute(
            'SELECT * FROM players WHERE server_id = ? AND discord_id = ? AND is_active = 0',
            [deadOpsServer.id, player.discord_id]
          );
          
          if (inactiveRecord.length > 0) {
            // Activate existing record
            await connection.execute(
              'UPDATE players SET is_active = 1, linked_at = NOW() WHERE id = ?',
              [inactiveRecord[0].id]
            );
            console.log(`    🔧 Activated existing Dead-ops record for ${player.ign}`);
            activatedCount++;
          } else {
            // Create new record
            await connection.execute(
        ...
        if (player.missingUsaDeadOps) {
          ...
          const [inactiveRecord] = await connection.execute(
            'SELECT * FROM players WHERE server_id = ? AND discord_id = ? AND is_active = 0',
            [usaDeadOpsServer.id, player.discord_id]
          );
          ...
          if (inactiveRecord.length > 0) {
            ...
            console.log(`    🔧 Activated existing USA-DeadOps record for ${player.ign}`);
            activatedCount++;
          } else {
            // Create new record
            await connection.execute(
        ...
        console.log(`    ❌ Failed to create/activate record for ${player.ign}: ${error.message}`);
      }
    }
    
    console.log(`\n  📊 Created ${createdCount} new records, activated ${activatedCount} existing records`);
    ...
    if (verifiedCount === allDiscordUsers.length) {
      ...
      console.log(`  Missing: ${allDiscordUsers.length - verifiedCount} players`);
    }
...
// Run the fix
fixMissingPlayerRecordsV2().catch(console.error);

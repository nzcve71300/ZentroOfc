-- Unified System Setup for pgAdmin 4
...
-- Step 2: Drop existing tables (if you want a clean slate)
...
-- Step 6: Migrate existing data (if you had any)
-- This will migrate your existing 17 players and 2 economy records
-- You can run this after creating the tables

-- Optional: If you want to migrate existing data, uncomment and run this:
/*
-- Migrate players (convert discord_id to BIGINT)
INSERT INTO players (guild_id, server_id, discord_id, ign, linked_at, is_active)
SELECT 
    guild_id, 
    server_id, 
    CAST(discord_id AS BIGINT), 
    ign, 
    NOW(), 
    true
FROM players_old
...
-- Step 7: Create link management tables

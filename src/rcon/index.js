...
function startRconListeners(client) {
  ...
  // Clean up expired state locks every 5 minutes
  setInterval(async () => {
    try {
      const [result] = await pool.query(`
        DELETE FROM zorp_state_locks WHERE expires_at < NOW()
      `);
      if (result.affectedRows > 0) {
        console.log(`[ZORP ROCK SOLID] Cleaned up ${result.affectedRows} expired state locks`);
      }
    } catch (error) {
      console.error('[ZORP ROCK SOLID] Error cleaning up expired locks:', error);
    }
  ...
  */
  ...
  setTimeout(() => {
    console.log('🚀 Initializing ZORP health check monitoring...');
    setInterval(() => {
      runZorpHealthCheck();
    }, 600000); // 10 minutes
...
// ===== ROCK-SOLID ZORP MONITORING SYSTEM =====
    ...
    for (const zone of zones) {
      try {
        ...
        const [hasLockResult] = await pool.query(`
          SELECT COUNT(*) as lock_count FROM zorp_state_locks 
          WHERE zone_id = ? AND expires_at > NOW()
        `, [zone.id]);
        
        const hasActiveLock = hasLockResult[0].lock_count > 0;
            ...
            if (onlineDuration >= 30 || currentState.last_online_at === null) {
              ...
              try {
                await pool.query(`
                  ...
                  VALUES (?, ?, 'green', 'PLAYER_ONLINE', DATE_ADD(NOW(), INTERVAL 2 MINUTE))
                ...
                await pool.query(`DELETE FROM zorp_state_locks WHERE zone_id = ?`, [zone.id]);
              } catch (lockError) {
                if (lockError.code === 'ER_DUP_ENTRY') {
                  console.log(`[ZORP ROCK SOLID] Could not acquire lock for zone ${zone.name} - already locked`);
                } else {
                  console.log(`[ZORP ROCK SOLID] Lock error for zone ${zone.name}:`, lockError.message);
                }
              }
            } else {
          ...
          if (currentState.desired_state === 'green' || currentState.current_state === 'green') {
            ...
            if (offlineDuration >= 60 || currentState.last_offline_at === null) {
              ...
              try {
                await pool.query(`
                  INSERT INTO zorp_state_locks (zone_id, current_state, target_state, transition_reason, expires_at)
                  VALUES (?, 'green', 'yellow', 'PLAYER_OFFLINE', DATE_ADD(NOW(), INTERVAL 2 MINUTE))
                `, [zone.id]);
            ...
            if (shouldTransition) {
              ...
              try {
                await pool.query(`
                  INSERT INTO zorp_state_locks (zone_id, current_state, target_state, transition_reason, expires_at)
                  VALUES (?, 'yellow', 'red', 'OFFLINE_TIMEOUT', DATE_ADD(NOW(), INTERVAL 5 MINUTE))
                `, [zone.id]);
...
/**
 * Rock-solid primary monitoring for all servers
 */
async function checkAllPlayerOnlineStatusRockSolid(client) {
  try {
    console.log('🔄 [ZORP ROCK SOLID] Checking player online status for all servers...');
    ...
    for (const server of result) {
      try {
        console.log(`📡 [ZORP ROCK SOLID] Checking online status for ${server.nickname}...`);
        await monitorZorpZonesRockSolid(client, server.guild_discord_id, server.nickname, server.ip, server.port, server.password, false);
      } catch (error) {
        console.error(`❌ [ZORP ROCK SOLID] Error checking online status for ${server.nickname}:`, error);
      }
    }
    
    console.log('✅ [ZORP ROCK SOLID] Player online status check completed');
  } catch (error) {
    console.error('❌ [ZORP ROCK SOLID] Error in checkAllPlayerOnlineStatusRockSolid:', error);
  }
}

/**
 * Rock-solid backup monitoring for all servers
 */
async function startBackupZorpMonitoringRockSolid(client) {
  console.log('🚀 [ZORP BACKUP ROCK SOLID] Starting backup ZORP monitoring system...');
  
  setInterval(async () => {
    try {
      console.log('🔄 [ZORP BACKUP ROCK SOLID] Running backup monitoring check...');
      ...
      for (const server of result) {
        try {
          console.log(`📡 [ZORP BACKUP ROCK SOLID] Backup check for ${server.nickname}...`);
          await monitorZorpZonesRockSolid(client, server.guild_discord_id, server.nickname, server.ip, server.port, server.password, true);
        } catch (error) {
          console.error(`❌ [ZORP BACKUP ROCK SOLID] Error in backup check for ${server.nickname}:`, error);
        }
      }
      
      console.log('✅ [ZORP BACKUP ROCK SOLID] Backup monitoring check completed');
    } catch (error) {

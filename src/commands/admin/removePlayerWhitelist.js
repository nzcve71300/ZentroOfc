...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('remove-player-whitelist')
    ...
    .addStringOption(option =>
      option.setName('playername')
        .setDescription('Player name to remove from whitelist')
        .setRequired(true)),
  ...
  async autocomplete(interaction) {
    ...
    try {
      ...
      console.error('Error in remove-player-whitelist autocomplete:', error);
  ...
  async execute(interaction) {
    ...
    try {
      ...
      if (existingResult.length === 0) {
        return interaction.editReply({
          embeds: [errorEmbed('Player Not Whitelisted', `**${playerName}** is not whitelisted for **${whitelistType.replace('_', ' ')}** on **${serverName}**.`)]
        });
      }

      // Remove player from whitelist
      await pool.query(
        'DELETE FROM player_whitelists WHERE guild_id = (SELECT id FROM guilds WHERE discord_id = ?) AND server_id = ? AND player_name = ? AND whitelist_type = ?',
        [guildId, serverId, playerName, whitelistType]
      );
      ...
      await interaction.editReply({
        embeds: [successEmbed(
          ...
          `**${playerName}** has been removed from the **${whitelistTypeDisplay}** whitelist for **${serverName}**.\n\nRemoved by: ${interaction.user.tag}`
        )]
      });

    } catch (error) {
      console.error('Error in remove-player-whitelist command:', error);
      await interaction.editReply({
        embeds: [errorEmbed('Error', `Failed to remove player from whitelist: ${error.message}`)]
      });
    }
  }
}; 
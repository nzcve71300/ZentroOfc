...
async function addRealServer() {
  try {
    ...
    const serverDetails = {
      id: '1753965211295_c5pfupu9',
      nickname: 'Rise 3x',
      ip: '149.102.132.219',
      port: 30216,
      password: 'JPMGiS0u', // This is the regular password field
      rcon_password: 'JPMGiS0u' // This is the RCON password field
    };
    ...
    if (existingServers.length > 0) {
      ...
      await pool.query(
        'UPDATE rust_servers SET ip = ?, port = ?, password = ?, rcon_password = ? WHERE id = ?',
        [serverDetails.ip, serverDetails.port, serverDetails.password, serverDetails.rcon_password, existingServers[0].id]
      );
      ...
      const [result] = await pool.query(
        ...
        [serverDetails.id, guildRecord.id, serverDetails.nickname, serverDetails.ip, serverDetails.port, serverDetails.password, serverDetails.rcon_password]
      );

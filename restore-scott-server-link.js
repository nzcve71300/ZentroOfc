...
/**
 * Restore Scott Server Link Script
 * 
 * This script restores the missing server entry for scott_T_BBK
 * so they can access both servers in their guild
 */

require('dotenv').config();
const pool = require('./src/db/index');

async function restoreScottServerLink() {
  try {
    console.log('🔧 Restoring scott_T_BBK server link...\n');
    ...
    // Get all servers in the DeadOps guild
    ...
    console.log(`\n🏰 Servers in guild ${guildId}:`);
    guildServers.forEach((server, index) => {
      console.log(`   ${index + 1}. ID=${server.id}, Name="${server.name}"`);
    });
    
    // Check which servers the player is missing
    const linkedServerIds = currentEntries.map(entry => entry.server_id);
    const missingServers = guildServers.filter(server => 
    ...
    if (missingServers.length > 0) {
      ...
      for (const server of missingServers) {
        console.log(`   Creating entry for ${server.name}...`);
